...
$(function(){
    ...
    $('#deleteFreeboardBTN').click(function(){
    	//var flag = redirectFlag();
    	//if (flag) {
    		location.href = '${pageContext.request.contextPath}/admin/freeboard/deleteFreeboard.do?free_no='+${freeInfo.free_no};
...
//
    ...
    $('#freeReplyRegist').submit(function(){
		...
		if(flag){
			$(this).attr('action', '${pageContext.request.contextPath}/admin/freereply/insertFreeReply.do');
			$(this).append('<input type="hidden" name="free_no" value="${freeInfo.free_no}"></input>');
	...
	//	}else if("${LOGIN_MEMBERINFO.mem_id}" == "${freeInfo.mem_id}"){
		//	var fr_no = $(this).attr('id');
			$(location).attr('href', '${pageContext.request.contextPath}/admin/freereply/deleteFreeReply.do?fr_no=' + fr_no);
		//}
	});
...
<form id="${replyList.fr_no}" method="post" action="${pageContext.request.contextPath}/admin/freereply/updateFreeReply.do">
		<div class="form-group"> 
		<label class="control-label col-sm-2" >댓글:</label>
		<div class="col-sm-10">
...
<form method="post" action="${pageContext.request.contextPath}/admin/freereply/insertFreeReReply.do">
		<div class="form-group">
