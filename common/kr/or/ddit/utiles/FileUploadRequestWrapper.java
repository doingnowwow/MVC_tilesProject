...
public class FileUploadRequestWrapper extends HttpServletRequestWrapper {
   ...
   // new FileUploadRequestWrapper(request, 1024*1024*100
   public FileUploadRequestWrapper(HttpServletRequest request,
                           int threshold,
                           int sizeMax){
      super(request);
               ...
               if(item.getSize() > 0){
                  ...
                  if(item.isFormField()){
                     ...
                     if(values == null){
                        ...
                        // 배열 카피 : 스왈로우 카피 - 원본 배열의 인덱스에서 레퍼런스하는 요소값의 주소가 복사.
                        //         MemberVO[] 원본 : [0] - MemberVO저장(주소1) => 스왈로우 카피 => MemberVO[] 복사 대상 배열 - [0] MemberVO(동일한 주소1 레퍼런스)
                        //                     [1] - MemberVO저장(주소2)                                [1] MemberVO(동일한 주소2 레퍼런스)
                         //         딥 카피 -원본 배열의 각 인덱스에서 레퍼런스 하는 요소값의 주소와 복사 대상 배열의 인덱스가 레퍼선스하는 요소값의 주소와 상이
                        //                      [0] - MemberVO저장(주소1) => 스왈로우 카피 => MemberVO[] 복사 대상 배열 - [0] MemberVO(동일한 주소3 레퍼런스)
                           //                    [1] - MemberVO저장(주소2)                                [1] MemberVO(동일한 주소4 레퍼런스)
                        // 1. 원본 배열
                        // 2. 원본 배열의 복사 시작될 인덱스
                        // 3. 복사 대상 배열
